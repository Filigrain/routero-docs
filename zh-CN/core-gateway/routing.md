...
| `auto_router` | 由 Routero 托管，随时间自适应 | 免维护优化 |

在你的 Router 配置或策略 YAML 中，为每个模型组设置策略。
...
发往 `model: "smart/balanced"` 的请求会分发到全部三个部署。发生故障时，Router 会自动在下一个可用部署上重试。
...
→ 关于影响路由决策的声明式 YAML 规则，参见 [策略路由]({% link zh-CN/core-gateway/policy-routing.md %})。
