...
每一次策略变更都是一个不可篡改的审计事件：`policy.changed`（v17 → v18），包括是谁、在何时做出的变更。审计日志提供一份可复现的记录，标明在任意时间点处于活跃状态的是哪个策略版本。

→ 关于完整的审计日志架构，参见 [SSO、RBAC 与审计]({% link zh-CN/core-gateway/sso-rbac-audit.md %})。

---

## 路由信号参考

关于全部 42 个路由信号及其语法的完整列表，参见 [API 参考]({% link zh-CN/api-reference.md %}) 中的 **Policy** 部分。
