...
- 受邀者通过邀请设置自己的访问权限，使用自己的凭据登录，或在工作区已配置单点登录时通过 SSO 登录。

{: .note }
Routero 支持通过 **Google**、**Microsoft / Azure AD** 以及通用 **OIDC** 进行单点登录，并通过 **SCIM** 实现用户自动开通。（不支持 SAML。）你的管理员可在 **Settings → SSO** 中配置。
...
可从仪表板或管理 API（`GET /audit`、`GET /audit/{id}`）查询审计日志，范围限定到你的组织。敏感值（如密钥内容）在存储前会被掩码处理。
