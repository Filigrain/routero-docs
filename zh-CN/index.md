...
只需一行代码修改 `base_url`，即可获得 100+ 模型、内置故障转移、声明式策略、支出管控，以及不可篡改的审计日志——并将数据严格保存在你的安全团队所要求的位置。
...
### 路由与故障转移
带有序供应商回退的命名模型组。在 5xx、限流或内容过滤触发时自动重试。支持流式——不丢任何数据块。一条链中放三家供应商，就是新的 99.99% 可用性。
...
### SSO、RBAC 与审计
SAML 2.0 · SCIM 2.0 自动开通 · Cerbos 细粒度授权 · 短时效的限定范围虚拟密钥 · 针对每个请求、策略变更和密钥轮换的不可篡改、加密签名审计日志。

[→ SSO、RBAC 与审计]({% link zh-CN/core-gateway/sso-rbac-audit.md %})
...
**安全与合规人员**——审查与批准。
从 [SSO、RBAC 与审计]({% link zh-CN/core-gateway/sso-rbac-audit.md %})、[合规]({% link zh-CN/security-trust/compliance.md %})和[部署选项]({% link zh-CN/deployment.md %})开始。
